...
} // You can do a 'console.log(product);' on this line and get the same output.
  // But since product is defined within the function (locally), if 'console.log(product);' is entered outside of the function, it will throw an error.
  // ReferenceError: product is not defined
  // If you want to check 'console.log(product);' for the viability of the code, it needs to be entered before 'return'
  return product;
} 
...
```JS
function checkEqual(a, b) {
  return a === b ? "Equal" : "Not Equal";  // use of equality operator (==) instead of the strict equality operator (===) is acceptable as well.
}
```
